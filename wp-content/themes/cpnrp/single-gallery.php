<?php
/**
 * Single gallery album — photo grid with lightbox.
 */

$post_id    = get_the_ID();
$ids_raw    = get_post_meta( $post_id, '_gallery_photos', true ) ?: '[]';
$photo_ids  = json_decode( $ids_raw, true );
if ( ! is_array( $photo_ids ) ) $photo_ids = [];

$date_label   = get_post_meta( $post_id, '_gallery_date', true );
$photo_count  = count( $photo_ids );
$archive_link = get_post_type_archive_link( 'gallery' ) ?: home_url( '/fotogalerie/' );

// Build photo data for JS
$photos = [];
...
<section class="events-page-hero">
	<div class="container">
		<nav class="events-page-hero-breadcrumbs" aria-label="<?php esc_attr_e( 'Navigace', 'cpnrp' ); ?>">
			...
			<a href="<?php echo esc_url( $archive_link ); ?>"><?php esc_html_e( 'Fotogalerie', 'cpnrp' ); ?></a>
			<span class="sep" aria-hidden="true">/</span>
			<span class="current"><?php the_title(); ?></span>
		</nav>
		<h1><?php the_title(); ?></h1>
		<?php if ( $date_label || $photo_count ) : ?>
		<p>
			<?php if ( $photo_count ) echo $photo_count . ' fotografií'; ?>
			<?php if ( $photo_count && $date_label ) echo ' &middot; '; ?>
	...
	<div class="container">
		...
		(function () {
			...
			function open(idx) {
				...
				show(idx);
				document.getElementById('gallery-lb-close').focus();
			}
			...
			document.querySelectorAll('.gallery-photo-item').forEach(function(btn) {
		...
		<div class="gallery-back-link">
			<a href="<?php echo esc_url( $archive_link ); ?>" class="btn btn--outline">
				<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true">
					<polyline points="15 18 9 12 15 6"/>
				</svg>
				<?php esc_html_e( 'Zpět na fotogalerie', 'cpnrp' ); ?>
			</a>
		</div>
	</div>
</section>
